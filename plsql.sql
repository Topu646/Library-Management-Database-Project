...
set serveroutput on
declare
req_date date := TO_DATE('2022-03-25', 'YYYY-MM-DD');
cursor c is select title from books where book_id in ( select book_id from bookrequests where request_date = req_date);
titlerow books.title%type;
begin
open c;
fetch c into titlerow;

while c%found 
loop
    dbms_output.put_line(titlerow);
    fetch c into titlerow;
